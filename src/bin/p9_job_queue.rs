...
use smol::{
    io::{AsyncBufReadExt, AsyncWriteExt, BufReader}, Async, Timer
};
use std::{collections::{BinaryHeap, HashMap}, net::{SocketAddr, TcpListener, TcpStream}, sync::{Arc, Mutex}, time::Duration};
    ...
    loop {
        ...
        let resp = match req {
            Request::Put { queue, job, pri } => {
                ...
                serde_json::to_string(&PutResponse { status: Status::Ok, id })?
            },
                ...
                let resp = if wait {
                    ...
                    loop {
                        match ctx.poll_job(&queues, addr) {
                            Some(job) => {
                                job_resp = GetResponse::Ok { id: job.id, job: job.data.clone(), pri: job.pri, queue: job.queue.clone() };
                                break
                            },
                            None => {
                    ...
                    match ctx.poll_job(&queues, addr) {
                        Some(job) => GetResponse::Ok { id: job.id, job: job.data.clone(), pri: job.pri, queue: job.queue.clone() },
...
impl Context {
        ...
        for queue_name in queues {
            if let Some(heap) = this.jobs_by_queue.get(queue_name) {
                if let Some(job) = heap.peek() {
                    if highest_priority_job.is_none() || job > &highest_priority_job.as_ref().unwrap() {
    ...
    fn try_delete(&self, job_id: i32) -> Result<(), ()> {
        ...
        let client_id = this.jobs_by_client_working.iter()
            .find(|(_, v)| v.id == job_id)
            .map(|(k,_)| k)
            .cloned();
        if let Some(k) = client_id {
            println!("def deleting for client_id {:?}", k);
